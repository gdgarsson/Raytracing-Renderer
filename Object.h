...
protected:
	...
	float m_shininess = 20.0f; // The "n" shininess value associated with specular highlights. The larger the number, the more focused the highlight.
	ShadeType m_shade_type = kPhong; // Determines which type of shading this object uses, is Phong by default
	float m_Ks = 0.4f; // Specular material coefficient
	float m_Kd = 1.0f; // Diffuse material coefficient
